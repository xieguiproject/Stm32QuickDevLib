#line 1 "..\\User\\SwitchControl.c"
 
#line 1 "..\\User\\SwitchControl.h"
...
{
	uint8_t ucBoardAddr;								 				  
	uint8_t ucBoardLockState[25];		  
}BoardInfo;

 
typedef struct tagCommunicationInfo_s
{
	uint8_t RecvMessage[50];					 
	uint8_t SendMessage[50];					 
	uint8_t RecvCount;													 
	uint8_t WorkFlage;													 
}CommunicationInfo;
typedef struct tagSwitchControl_s
{
	BoardInfo sAllBoardInfo[6];					 
	CommunicationInfo sComInfo;
}SwitchControl;
...
extern uint8_t SwitchControlInit();
extern uint16_t SwitchControlDel();
extern uint16_t OpenALock(uint8_t BoardAddr,uint8_t LockAddr);
#line 3 "..\\User\\SwitchControl.c"
...
#line 6 "..\\User\\SwitchControl.c"

SwitchControl mdSwitchControl;
...
uint16_t OpenALock(uint8_t BoardAddr,uint8_t LockAddr)
{
	OpenBoardLock OpenBoardLockCmd;
	
	if(BoardAddr < 1 || BoardAddr > 6)
	{
		return 0;
	}
	
	if(LockAddr < 1 || LockAddr > 25)
	
	OpenBoardLockCmd.ucHead = 0X8A;
	...
	USARTWrite(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)),(char *)&OpenBoardLockCmd,sizeof(OpenBoardLock));
}
 
uint16_t OpenAllLock()
{
	uint8_t  ucLoop = 0;
	OpenBoardLock OpenBoardLockCmd;
	
	for(ucLoop = 1; ucLoop < 6; ucLoop++)
	{
		OpenBoardLockCmd.ucHead = 0X8A;
		OpenBoardLockCmd.ucBoardAddr = ucLoop;
		OpenBoardLockCmd.ucLockAddr = 0x00;
		OpenBoardLockCmd.ucState = 0x11;
		OpenBoardLockCmd.ucCrc = CalcCrc((char *)&OpenBoardLockCmd,(sizeof(OpenBoardLock) -1));
...
uint16_t SetBoardALockState(uint8_t BoardAddr,uint8_t LockAddr,uint8_t State)
{
	mdSwitchControl.sAllBoardInfo[BoardAddr].ucBoardLockState[LockAddr] = State;
	
	return 1;
}
 
uint16_t OpenLockRspDel(OpenBoardLockRsp *aBoardLockState)
{
	 
	if(aBoardLockState->ucState == 0X11)
	{
		SetBoardALockState(aBoardLockState->ucBoardAddr,aBoardLockState->ucLockAddr,
												aBoardLockState->ucState);
		return 1;
	}
...
uint16_t SetBoardLockState(uint8_t *ucBoardLockState,ReadBoardLockStateRsp *aBoardLockState)
{
	uint8_t uiLoop =0;
	uint32_t Temp = 0;
	
	Temp = ((uint32_t)aBoardLockState->ucState1 * 256 + aBoardLockState->ucState2) * 256 + aBoardLockState->ucState3;

	 
	for(uiLoop = 1;uiLoop < 25; uiLoop++)
	{
		if((Temp && (0x000001 << (uiLoop - 1))))
		{
			 
			ucBoardLockState[uiLoop] =  0X11;
			
		}
		else
		{
			ucBoardLockState[uiLoop] =  0X00;
		}
	}
	return 1;
}
 
uint16_t ReadLockRspDel(ReadBoardLockStateRsp *aBoardLockState)
{
	 
	uint8_t uiLoop = 0;
	
	SetBoardLockState(mdSwitchControl.sAllBoardInfo[aBoardLockState->ucBoardAddr].ucBoardLockState,
										aBoardLockState);

}
 
uint16_t SwitchControlBoardRspDel()
{
	ReadBoardLockStateRsp *aBoardLockState = 0;
	OpenBoardLockRsp      *aOpenBoardLockRspState = 0;
	
	if(mdSwitchControl.sComInfo.RecvMessage[0] != 0X81 && 
		 mdSwitchControl.sComInfo.RecvMessage[0] != 0X8A &&
		 mdSwitchControl.sComInfo.RecvMessage[0] != 0X80 &&
		 mdSwitchControl.sComInfo.RecvMessage[0] != 0X90)
	{
		return -1; 
	}
	
	switch(mdSwitchControl.sComInfo.RecvMessage[0])
	{
		...
		case 0X8A:		 
				...
				aOpenBoardLockRspState = (OpenBoardLockRsp *)&mdSwitchControl.sComInfo.RecvMessage[0];
				OpenLockRspDel(aOpenBoardLockRspState);	
				break;
		 
		case 0X80:
				aBoardLockState = (ReadBoardLockStateRsp *)&mdSwitchControl.sComInfo.RecvMessage[0];
				ReadLockRspDel(aBoardLockState);
...
uint16_t SwitchControlDel()
{
	if(mdSwitchControl.sComInfo.WorkFlage & 0X02)			 
	{
		if(mdSwitchControl.sComInfo.WorkFlage & 0X04)	 
		{
			mdSwitchControl.sComInfo.WorkFlage &= ~0X02; 
			
			SwitchControlBoardRspDel();
			mdSwitchControl.sComInfo.WorkFlage &= ~0X04; 
			mdSwitchControl.sComInfo.RecvCount = 0;
...
uint16_t SwitchControlRecvCmdDel(uint16_t arg1,uint16_t arg2)
{
	mdSwitchControl.sComInfo.WorkFlage |= 0X02;	 
	
	mdSwitchControl.sComInfo.RecvMessage[mdSwitchControl.sComInfo.RecvCount] = arg1;
	
	mdSwitchControl.sComInfo.RecvCount++;
	
	if(mdSwitchControl.sComInfo.RecvCount > 2)
	{
		mdSwitchControl.sComInfo.WorkFlage |= 0X04;
	}
	if(mdSwitchControl.sComInfo.RecvCount > 50)
	{
...
uint8_t SwitchControlInit()
{
	memset(&mdSwitchControl, 0, sizeof(SwitchControl));
	
	USARTInit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), 9600, ((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)), ((uint16_t)0x0200), ((uint16_t)0x0400));
	
	
	mdSwitchControl.sAllBoardInfo[0].ucBoardAddr =0x01;
	mdSwitchControl.sAllBoardInfo[1].ucBoardAddr =0x02;
	mdSwitchControl.sAllBoardInfo[2].ucBoardAddr =0x03;
	mdSwitchControl.sAllBoardInfo[3].ucBoardAddr =0x04;
	mdSwitchControl.sAllBoardInfo[4].ucBoardAddr =0x05;
	
	mdUsart.USART1_s.USARTUserDel = SwitchControlRecvCmdDel; 
}
