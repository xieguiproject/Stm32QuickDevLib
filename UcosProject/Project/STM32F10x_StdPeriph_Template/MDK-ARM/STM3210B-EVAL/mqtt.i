...
uint16_t MQTTConnectServer(uint16_t KeepOnlineTime,char *MessageBuf,uint16_t *MessageLen,
													char *DeviceId,char *UserName,char *Password)
 
{
	MQTTConnect MQTTConnectPar;
	char ucDeviceIdBuff[25] = {0}; 
	char ucUserNameBuff[25] = {0}; 
	char ucPasswordBuff[25] = {0}; 
	uint8_t DeviceIdLen = 0;	 
	uint8_t UserNmaeLen = 0;
	uint8_t PasswordlLen =0;
	
	
	if(DeviceId == 0 || UserName == 0 || Password == 0)
	{
		{ if(1 == 1) { printf("DeviceID Or UserName Or Password Is NULL\n"); }};
		return 0; 
	}
	
	 
	MQTTConnectPar.sHeader.ucPacketType |= (MQTT_PKT_CONNECT << 4); 
	MQTTConnectPar.sHeader.ucRemainingLenth = (sizeof(MQTTConnect) - 2) + DeviceIdLen +2 + UserNmaeLen +2 + PasswordlLen + 2; 
	...
	MQTTConnectPar.uiMqttName[3] = 'T'; 
	MQTTConnectPar.ucProtocolLevel = 0x04;
	MQTTConnectPar.ucConnectFlag |= (0X80 | 0X40); 
	MQTTConnectPar.ucKeepAliveTime = ConvDat(KeepOnlineTime); 
	...
	if(DeviceIdLen > 25 || UserNmaeLen > 25 || DeviceIdLen > 25)
	{
		{ if(1 == 1) { printf("DeviceIdLen Or UserNmaeLen Or DeviceIdLen overflow\n"); }};
		return 0; 
	}
	...
	memcpy(MessageBuf,&MQTTConnectPar,sizeof(MQTTConnect)); 
	
	memcpy(&MessageBuf[sizeof(MQTTConnect)],ucDeviceIdBuff,DeviceIdLen + 2); 
	
	memcpy(&MessageBuf[sizeof(MQTTConnect) + DeviceIdLen +2 ],ucUserNameBuff,UserNmaeLen + 2); 
	
	memcpy(&MessageBuf[sizeof(MQTTConnect) + DeviceIdLen +2 + UserNmaeLen +2 ],ucPasswordBuff,PasswordlLen + 2); 
	
	*MessageLen = sizeof(MQTTConnect) + DeviceIdLen +2 + UserNmaeLen +2 + PasswordlLen + 2;
	
	return 1;
}
uint16_t MQTTPublist(char *TopicName,char *MessageBuf,uint16_t *MessageLen)
{
	MQTTPush MQTTPushStr;
	char TopicNameBuf[25] = {0};
	...
	MQTTPushStr.sHeader.ucRemainingLenth = 0;
	...
	memcpy(&TopicNameBuf[2],TopicName,TopicNameLen); 
	
	memcpy(MessageBuf,&MQTTPushStr,sizeof(MQTTPush)); 
	
	memcpy(&MessageBuf[sizeof(MQTTPush)],TopicNameBuf,TopicNameLen); 
